...
function downloadNoteAsText(note, projectName) {
  const blob = new Blob([noteAsText(note, projectName)], { type: "text/plain" });
...
export default function App() {
  ...
  const loadAll = async () => {
    setLoading(true);
    setLoadError("");
    const [{ data: proj, error: e1 }, { data: nts, error: e2 }] = await Promise.all([
  ...
  const toggleActionItem = async (note, idx) => {
    ...
    const { error } = await supabase.from("notes").update({ action_items: items }).eq("id", note.id);
    if (!error) setNotes((prev) => prev.map((n) => (n.id === note.id ? { ...n, action_items: items } : n)));
  };
  ...
  const stats = useMemo(() => {
    const decisionsThisWeek = notes.filter((n) => isThisWeek(n.created_at)).reduce((s, n) => s + (n.decisions || []).length, 0);
    const pendingTasks = notes.reduce((s, n) => s + (n.action_items || []).filter((a) => !a.done).length, 0);
    return { totalNotes: notes.length, decisionsThisWeek, pendingTasks, activeProjects: projects.length };
  }, [notes, projects]);
  ...
  const filteredNotes = notes.filter((n) => {
    ...
    const matchesSearch =
      !q || (n.summary || "").toLowerCase().includes(q) || projName.toLowerCase().includes(q) || (n.tags || []).some((t) => t.toLowerCase().includes(q));
  ...
  const allTasks = useMemo(() => {
    ...
    notes.forEach((n) => {
      const projName = projects.find((p) => p.id === n.project_id)?.name || "\u2014";
      (n.action_items || []).forEach((a, idx) => out.push({ ...a, idx, note: n, projName }));
    });
    return out.sort((a, b) => (a.done === b.done ? 0 : a.done ? 1 : -1));
  }, [notes, projects]);
  ...
  return (
    <div className="app-shell">
      ...
      <aside className={`sidebar ${sidebarOpen ? "open" : ""}`}>
        <div style={{ padding: "22px 20px 14px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <div style={{ width: 30, height: 30, borderRadius: 10, background: "var(--purple-grad)", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <Building2 size={15} color="#fff" strokeWidth={2.4} />
            </div>
            <span className="display" style={{ fontWeight: 800, fontSize: 19, letterSpacing: "-0.02em" }}>NoteArch</span>
            <button className="hamburger-btn" onClick={() => setSidebarOpen(false)} style={{ marginLeft: "auto", background: "none", border: "none", color: "var(--muted)" }}>
              <X size={18} />
            </button>
          </div>
        ...
        <div style={{ padding: "4px 14px", flex: 1, overflowY: "auto" }}>
          ...
          {projects.map((p) => {
            ...
            return (
              <SidebarItem key={p.id} label={p.name} count={notes.filter((n) => n.project_id === p.id).length} active={activeProject === p.name} dotColor={c.fg}
                onClick={() => { setActiveProject(p.name); setSidebarOpen(false); }} />
            );
          })}

          {showAddProject ? (
            <div style={{ padding: "6px 10px", display: "flex", gap: 6 }}>
              <input autoFocus value={newProjectInput} onChange={(e) => setNewProjectInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && addProject()}
      ...
      <div className="main-col">
        <div className="topbar" style={{ padding: "18px 24px 0", display: "flex", alignItems: "center", gap: 12 }}>
          <button className="hamburger-btn" onClick={() => setSidebarOpen(true)} style={{ background: "#fff", border: "1px solid var(--line)", borderRadius: 12, width: 40, height: 40, display: "flex", alignItems: "center", justifyContent: "center", color: "var(--ink)" }}>
            <Menu size={18} />
          </button>
          <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 8, background: "#fff", border: "1px solid var(--line)", borderRadius: 14, padding: "10px 14px" }}>
            <Search size={16} color="var(--muted-soft)" />
        ...
        <div className="hero-card">
          <svg className="hero-doodle" width="120" height="90" viewBox="0 0 120 90" fill="none">
            <circle cx="95" cy="20" r="18" stroke="white" strokeOpacity="0.5" strokeWidth="2" />
            <path d="M20 60 Q35 40 50 60 T80 60" stroke="white" strokeOpacity="0.5" strokeWidth="2" fill="none" />
            <circle cx="30" cy="15" r="4" fill="white" fillOpacity="0.5" />
          </svg>
        ...
        <div className="pill-tabs">
          ...
          {projects.map((p) => (
            <button key={p.id} className={`pill ${activeProject === p.name ? "active" : ""}`} onClick={() => setActiveProject(p.name)}>
              {p.name} <span style={{ opacity: 0.6 }}>{notes.filter((n) => n.project_id === p.id).length}</span>
            </button>
          ))}
        </div>
        ...
        <div className="main-scroll" style={{ flex: 1, overflowY: "auto", padding: "16px 24px 28px" }}>
          {loading ? (
            ...
            <div className="notes-grid">
              {filteredNotes.map((n) => (
                <NoteCard key={n.id} note={n} projectName={projects.find((p) => p.id === n.project_id)?.name || "\u2014"}
                  isOpen={openNoteId === n.id} onToggle={() => setOpenNoteId(openNoteId === n.id ? null : n.id)}
                  onDelete={() => deleteNote(n.id)} onToggleAction={(idx) => toggleActionItem(n, idx)} onSaveEdit={saveEditedNote} />
              ))}
            </div>
          )}
        </div>
      ...
      {showRecorder && <RecorderModal projects={projects} onClose={() => setShowRecorder(false)} onSaved={onNoteSaved} />}
    </div>
  );
}
...
function HeroStat({ value, label }) {
  return (
    <div>
      <div className="display" style={{ fontSize: 18, fontWeight: 800 }}>{value}</div>
...
function TabBtn({ active, onClick, icon, label, badge }) {
  return (
    <button onClick={onClick} style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "8px 8px", borderRadius: 12, border: "none", background: active ? "var(--purple-soft)" : "#f7f5ff", color: active ? "var(--purple)" : "var(--muted)", fontSize: 12.5, fontWeight: 700 }}>
      {icon} {label}
      {badge ? <span style={{ fontSize: 10, background: "var(--coral)", color: "#fff", borderRadius: 8, padding: "1px 5px" }}>{badge}</span> : null}
    </button>
  );
}
...
function StatCard({ label, value, accent }) {
  return (
    <div className="note-card">
      <div className="display" style={{ fontSize: 22, fontWeight: 800, color: accent }}>{value}</div>
      <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 2 }}>{label}</div>
    </div>
  );
}

function SidebarItem({ label, count, active, onClick, dotColor }) {
  return (
    <button onClick={onClick} style={{ width: "100%", display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 10px", borderRadius: 10, border: "none", background: active ? "var(--purple-soft)" : "transparent", color: active ? "var(--purple)" : "var(--ink)", fontSize: 13.5, fontWeight: active ? 700 : 500, marginBottom: 2, textAlign: "left" }}>
      <span style={{ display: "flex", alignItems: "center", gap: 8, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
        {dotColor && <span style={{ width: 7, height: 7, borderRadius: "50%", background: dotColor, flexShrink: 0 }} />}
        {label}
      </span>
      <span style={{ fontSize: 11, color: "var(--muted-soft)" }}>{count}</span>
    </button>
  );
}
...
function EmptyState({ icon, text }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 10, padding: "70px 20px", color: "var(--muted-soft)", fontSize: 13.5 }}>
      {icon}
      <span>{text}</span>
    </div>
  );
...
function NoteCard({ note, projectName, isOpen, onToggle, onDelete, onToggleAction, onSaveEdit }) {
  ...
  const saveEdit = async () => {
    const newDecisions = draft.decisions.split("\n").map((s) => s.trim()).filter(Boolean);
  ...
  return (
    <div className="note-card fade-up">
      <div onClick={onToggle} style={{ display: "flex", alignItems: "flex-start", gap: 12, cursor: "pointer" }}>
        {note.image_url ? (
          <img src={note.image_url} alt="" className="icon-chip" style={{ objectFit: "cover" }} />
        ) : (
          <div className="icon-chip" style={{ background: c.bg, color: c.fg }}>
            <FileText size={18} />
          </div>
        )}
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            ...
            <span style={{ fontSize: 11.5, color: "var(--muted-soft)" }}>{fmtDate(note.created_at)}</span>
          </div>
          <div style={{ fontSize: 13.5, marginTop: 5, color: "var(--muted)", lineHeight: 1.5 }}>{note.summary}</div>
          {note.attendees && (
            <div style={{ fontSize: 11.5, color: "var(--purple)", marginTop: 4, display: "flex", alignItems: "center", gap: 4 }}>
              <Users size={11} /> {note.attendees}
            </div>
          )}
          <div style={{ display: "flex", gap: 6, marginTop: 9, flexWrap: "wrap", alignItems: "center" }}>
            {decisions.length > 0 && (
              <span className="tag-pill" style={{ background: "var(--green-soft)", color: "var(--green)" }}>
                {decisions.length} decision{decisions.length > 1 ? "s" : ""}
              </span>
            )}
            {tags.slice(0, 3).map((t) => (
              <span key={t} className="tag-pill" style={{ background: "var(--purple-soft)", color: "var(--purple)" }}>{t}</span>
            ))}
          </div>
        </div>
        {isOpen ? <ChevronDown size={17} style={{ marginTop: 8, flexShrink: 0 }} /> : <ChevronRight size={17} style={{ marginTop: 8, flexShrink: 0 }} />}
      </div>

      {isOpen && (
        <div style={{ paddingTop: 14, marginTop: 14, borderTop: "1px solid var(--line)", display: "flex", flexDirection: "column", gap: 14 }}>
          {note.image_url && (
            <img src={note.image_url} alt="Attached" style={{ width: "100%", maxHeight: 220, objectFit: "cover", borderRadius: 14 }} />
          )}
          {editing ? (
            <>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={saveEdit} style={{ background: "var(--purple-grad)", color: "#fff", border: "none", borderRadius: 10, padding: "8px 16px", fontSize: 12.5, fontWeight: 700 }}>Save changes</button>
                <button onClick={() => setEditing(false)} style={{ background: "none", border: "1px solid var(--line)", borderRadius: 10, padding: "8px 16px", fontSize: 12.5, color: "var(--muted)" }}>Cancel</button>
              </div>
            </>
          ) : (
            <>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
                {actionItems.map((a, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13 }}>
                    <button onClick={() => onToggleAction(i)} style={{ background: "none", border: "none", padding: 0, color: a.done ? "var(--green)" : "var(--muted-soft)", display: "flex" }}>
          ...
          <Section title="Full transcript">
            <div style={{ fontSize: 12.5, color: "var(--muted)", lineHeight: 1.7, whiteSpace: "pre-wrap", background: "#faf9ff", border: "1px solid var(--line)", borderRadius: 12, padding: 12, maxHeight: 180, overflowY: "auto" }}>
              {note.transcript}
            </div>
          </Section>
...
function Section({ title, icon, children }) {
  return (
    <div>
      <div style={{ fontSize: 11, fontWeight: 700, color: "var(--muted-soft)", letterSpacing: "0.04em", marginBottom: 6, display: "flex", alignItems: "center", gap: 5 }}>
...
function RecorderModal({ projects, onClose, onSaved }) {
  ...
  useEffect(() => {
    ...
    if (!SR) {
      setMicError("Voice-to-text isn't supported in this browser. Type the discussion instead.");
      setMode("type");
      return;
    }
    const rec = new SR();
  ...
  const saveNote = async () => {
    ...
    try {
      ...
      if (photoFile) {
        ...
        const { error: upErr } = await supabase.storage.from("note-photos").upload(path, photoFile);
        if (!upErr) {
          const { data: pub } = supabase.storage.from("note-photos").getPublicUrl(path);
      ...
      setStatus("error");
      setErrorMsg(err.message || "Couldn't save that note. Try again.");
    }
  };
  ...
  return (
    <div className="modal-overlay" style={{ position: "fixed", inset: 0, background: "rgba(33,28,52,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50, padding: 20 }}>
      <div className="modal-card" style={{ background: "#fff", borderRadius: 26, width: "100%", maxWidth: 560, boxShadow: "0 24px 70px rgba(33,28,52,0.3)", overflow: "hidden" }}>
        ...
        <div style={{ padding: 22, display: "flex", flexDirection: "column", gap: 16, maxHeight: "75vh", overflowY: "auto" }}>
          <div>
            <label style={{ fontSize: 11.5, fontWeight: 700, color: "var(--muted)" }}>PROJECT</label>
            <select value={projectId} onChange={(e) => setProjectId(e.target.value)}
              style={{ width: "100%", marginTop: 6, padding: "11px 12px", borderRadius: 12, border: "1px solid var(--line)", fontSize: 14, background: "#fff" }}>
              <option value="" disabled>{projects.length ? "Select a project" : "No projects yet — add one first"}</option>
              {projects.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
            </select>
          </div>

          <div>
            <label style={{ fontSize: 11.5, fontWeight: 700, color: "var(--muted)" }}>ATTENDEES (optional)</label>
            <input value={attendees} onChange={(e) => setAttendees(e.target.value)} placeholder="e.g. Client, Sachin, Design team"
              style={{ width: "100%", marginTop: 6, padding: "10px 12px", borderRadius: 12, border: "1px solid var(--line)", fontSize: 13.5 }} />
          </div>

          <div style={{ display: "flex", gap: 8 }}>
            <ModeTab active={mode === "voice"} onClick={() => setMode("voice")} icon={<Mic size={13} />} label="Voice" />
            <ModeTab active={mode === "type"} onClick={() => setMode("type")} icon={<Type size={13} />} label="Type" />
          </div>

          {mode === "voice" && (
            <div style={{ display: "flex", gap: 8, justifyContent: "center" }}>
              {LANGUAGES.map((l) => (
                <button key={l.code} onClick={() => setLang(l.code)} disabled={recording}
                  style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 12px", borderRadius: 999, border: "1px solid " + (lang === l.code ? "var(--purple)" : "var(--line)"), background: lang === l.code ? "var(--purple-soft)" : "#fff", color: lang === l.code ? "var(--purple)" : "var(--muted)", fontSize: 12, fontWeight: 600 }}>
                  <Languages size={11} /> {l.label}
                </button>
              ))}
            </div>
          )}

          {micError && <div style={{ fontSize: 12, color: "#b9791f", background: "var(--yellow-soft)", padding: "9px 12px", borderRadius: 10 }}>{micError}</div>}
          ...
          {mode === "voice" && (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10, padding: "12px 0" }}>
              <button onClick={recording ? stopRecording : startRecording} style={{
                ...
                background: recording ? "linear-gradient(135deg,#ff8b8b,#ff6b6b)" : "var(--purple-grad)",
                color: "#fff", display: "flex", alignItems: "center", justifyContent: "center",
                animation: recording ? "pulseRing 1.6s infinite" : "none",
                boxShadow: "0 10px 24px rgba(124,92,252,0.35)",
              }}>
          ...
          <div>
            <label style={{ fontSize: 11.5, fontWeight: 700, color: "var(--muted)" }}>SITE / MATERIAL PHOTO (optional)</label>
            <div style={{ marginTop: 6, display: "flex", alignItems: "center", gap: 10 }}>
              {photoPreview ? (
                <img src={photoPreview} alt="" style={{ width: 48, height: 48, borderRadius: 12, objectFit: "cover" }} />
              ) : (
                <label style={{ display: "flex", alignItems: "center", gap: 6, padding: "9px 14px", borderRadius: 12, border: "1px dashed var(--line)", color: "var(--muted)", fontSize: 12.5, cursor: "pointer" }}>
                  <Camera size={14} /> Add photo
                  <input type="file" accept="image/*" onChange={onPhotoSelected} style={{ display: "none" }} />
                </label>
              )}
              {photoPreview && (
                <button onClick={() => { setPhotoFile(null); setPhotoPreview(""); }} style={{ fontSize: 12, color: "var(--coral)", background: "none", border: "none" }}>Remove</button>
              )}
            </div>
          </div>

          {errorMsg && <div style={{ fontSize: 12, color: "var(--coral)" }}>{errorMsg}</div>}
...
function ModeTab({ active, onClick, icon, label }) {
  return (
    <button onClick={onClick} style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "9px 10px", borderRadius: 12, border: "1px solid " + (active ? "var(--purple)" : "var(--line)"), background: active ? "var(--purple-soft)" : "#fff", color: active ? "var(--purple)" : "var(--muted)", fontSize: 13, fontWeight: 700 }}>
      {icon} {label}
    </button>
  );
}
